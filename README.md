...
Welcome to the AI Coding Workshop! Over these 4 hours, you'll master the complete workflow for working with AI coding assistants and 10x the speed and reliability of your coding. This repository also includes the full production-ready AI agent template from the Dynamous AI Agent Mastery course that I built on top of for the workshop. The full Stripe implementation is here in the codebase - exactly what we built live together!

> Note: All workshop materials including the full README I'll cover during the event and the custom library of slash commands and subagents is in the `workshop_materials/` folder. This README is focused on the AI Agent Template (overview & setup) that we'll build on top of together.
...
You'll need a AI coding assistants to follow along in the workshop:
...
## Workshop Project: AI Agent SaaS Platform

Throughout this workshop, we'll work with a mature codebase - a modular, production-ready AI agent deployment system with:
...
- **Database**: Supabase with PostgreSQL
- **RAG**: Retrieval-Augmented Generation for context-aware responses
- **Payments**: Stripe for token-based billing and subscriptions
...
**Note for Workshop Participants:** Setting up the full AI agent is optional for the workshop. You don't need to have everything configured ahead of time to follow along with the workshop content. However, if you want to run the actual AI agent locally and see it in action, follow the setup guide below.
...
**Note:** This section is not necessary for the workshop, but if you want to take this AI agent and actually deploy it into production after the workshop, we provide comprehensive deployment options below.
...
- **Setup Guide**: See [`~deployment_guides~/cloud-ai-digitalocean.md`](~deployment_guides~/cloud-ai-digitalocean.md) for step-by-step instructions
- **Alternative**: For integration with Local AI Package, see [`~deployment_guides~/local-ai-digitalocean.md`](~deployment_guides~/local-ai-digitalocean.md)
...
- **Setup Guide**: See [`~deployment_guides~/render-deployment.md`](~deployment_guides~/render-deployment.md) for detailed instructions
...
- **Setup Guide**: See [`~deployment_guides~/gcp-deployment.md`](~deployment_guides~/gcp-deployment.md) for comprehensive setup
...
See `workshop_materials/` for the main workshop resources.